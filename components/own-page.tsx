...
function OwnerInfo() {
 const {market}=useCanvas();
 return <><span className="eyebrow">OWNER INFORMATION</span><SheetTitle className="s-preview-title">Set up your marketplace.</SheetTitle><SheetDescription className="s-preview-description">Deploy the Own This Page program with your admin wallet, then verify it before opening purchases.</SheetDescription><div className="owner-summary"><Brand/><dl><div><dt>Spaces</dt><dd>62 buyable placements</dd></div><div><dt>Official token CA</dt><dd>{officialProject.coinAddress?'Published':'Awaiting your new address'}</dd></div><div><dt>Slot purchases</dt><dd>{market.live?'Active':market.status==='error'?'Verification unavailable':'Not activated'}</dd></div><div><dt>Network</dt><dd>Solana mainnet</dd></div></dl></div><p className="owner-copy">Your developer, treasury, and admin wallet:</p><a className="treasury-address" href={`${officialProject.explorer}/account/${solanaProject.treasury}`} target="_blank" rel="noopener noreferrer">{solanaProject.treasury}<ArrowUpRight size={15}/></a><p className="owner-copy">A token launch is optional. The marketplace has its own program address and uses SOL for payments.</p><a className="wallet-button" href="/?setup=1">Open marketplace setup<ArrowUpRight size={15}/></a></>;
}
 ...
 return <Context.Provider value={{market,ownedOnly:ownedOnly&&!!market.session,select}}><div className={`own-page ${showPrices?'show-prices':''}`}><Toaster theme="light" position="top-center"/><a className="skip-link" href="#canvas">Skip to the page</a>
  ...
  <main className="container"><h1 className="sr-only">Own This Page — 62 buyable spaces</h1><OfficialBar/>
   <section className="canvas-shell" id="canvas" aria-label="62 buyable spaces"><div className="canvas-tools"><div><strong>62 spaces</strong><span className="market-stats">0.1–2 SOL <span>start</span><span className="stats-divider">·</span>2× <span>takeovers</span></span><span className={`availability ${market.live?'is-live':''}`}>{market.live?`${claimed} claimed`:market.status==='error'?'Connection unavailable':'Preview'}</span></div><div className="canvas-tool-actions">{market.session&&<button className={ownedOnly?'control-active':''} aria-pressed={ownedOnly} onClick={()=>setOwnedOnly(!ownedOnly)}>My spaces <span>{owned}</span></button>}<button className={showPrices?'control-active':''} aria-pressed={showPrices} onClick={()=>setShowPrices(!showPrices)}><Grid2X2 size={15}/>{showPrices?'Hide prices':'Show prices'}</button></div></div>
    {!market.live&&<div className="preview-notice" id="market-availability"><span>{market.status==='error'?'The marketplace could not be verified. Purchases are paused.':'Purchases not open yet.'}</span><button onClick={()=>setRulesOpen(true)}>How it works <ArrowUpRight size={13}/></button></div>}
  ...
  <Sheet open={rulesOpen} onOpenChange={setRulesOpen}><SheetContent className="solana-preview-panel"><span className="eyebrow">A SHARED SPACE, SIMPLE RULES</span><SheetTitle className="s-preview-title">How this page works.</SheetTitle><SheetDescription className="s-preview-description">62 individual advertising spaces on Solana. The official brand, token address, and X account always stay protected.</SheetDescription><div className="rules-detail">{[
   ['Pick your space','Choose one of 62 spaces for your text, image, or link. Connect your Solana wallet and review the price before buying.'],
   ['Starting prices','Spaces start at 0.1–2 SOL. Network fees and any first-time storage deposit are shown before you approve.'],
   ['Make it yours','Edit your content while you own the space. Upload an image directly or use an image link. Editing has no placement fee; network fees still apply.'],
   ['Takeovers','Anyone can take over a space for twice its last price. All purchase and takeover payments go to the treasury; previous owners receive no payout.'],
  ].map(([q,a])=><section key={q}><h3>{q}</h3><p>{a}</p></section>)}</div></SheetContent></Sheet>
